...
class SessionDB:
    ...
    def update_session_model_config(self, session_id: str, model_config: Dict[str, Any]) -> bool:
        """Replace a session's model_config JSON blob."""
        payload = json.dumps(model_config) if model_config is not None else None
    ...
    def update_codex_lane_refs(
        ...
        *,
        ...
        """Store lightweight Hermes-owned Codex lane references on the session."""
        model_config = self.get_session_model_config(session_id)
        model_config["execution_backend"] = execution_backend
        model_config["codex_lane_ref"] = {
            "lane_name": lane_name,
            "thread_id": thread_id,
        ...
        return self.update_session_model_config(session_id, model_config)
