...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        # Local custom endpoints used as the main chat provider should not
        # block normal text sessions on vision preflight. For lightweight
        # local lanes (e.g. Ollama-backed Archimedes), vision is optional and
        # can be resolved explicitly later if needed.
        main_provider = _read_main_provider()
        if main_provider == "custom":
            logger.debug("Vision auto-detect: skipping custom main provider preflight")
            return None, None, None
