...
class TestGatewayKnownCommands:
    ...
    def test_codex_in_gateway(self):
        assert "codex" in GATEWAY_KNOWN_COMMANDS
...
class TestSubcommands:
    ...
    def test_codex_has_subcommands(self):
        assert "/codex" in SUBCOMMANDS
        subs = SUBCOMMANDS["/codex"]
        assert subs == ["run", "inspect", "list"]
...
class TestSubcommandCompletion:
    ...
    def test_codex_subcommand_completion_after_space(self):
        completions = _completions(SlashCommandCompleter(), "/codex ")
        texts = {c.text for c in completions}
        assert texts == {"run", "inspect", "list"}
